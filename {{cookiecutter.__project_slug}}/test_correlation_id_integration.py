...
async def test_all_tools(server_script_path: str):
    ...
    server_params = StdioServerParameters(
        command="python",
        args=["-m", "{{cookiecutter.__project_slug}}.server.app", "--transport", "stdio"],
        env=env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            console.print("1. Run the Streamlit UI: [cyan]streamlit run {{cookiecutter.__project_slug}}/ui/app.py[/cyan]")
            ...
            db_path = Path.home() / ".local" / "share" / "{{cookiecutter.__project_slug}}" / "unified_logs.db"
...
@click.option(
    '--server-module',
    default="{{cookiecutter.__project_slug}}.server.app",
