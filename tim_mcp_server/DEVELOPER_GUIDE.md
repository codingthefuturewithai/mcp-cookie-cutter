...
```python
# tim_mcp_server/tools/my_tools.py
...
```python
from tim_mcp_server.tools.my_tools import my_tools, my_parallel_tools
```
...
```bash
# Start the inspector
uv run mcp dev tim_mcp_server/server/app.py
...
uv run pytest --cov=tim_mcp_server --cov-report=html
