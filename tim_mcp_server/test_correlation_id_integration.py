...
def get_sqlite_logs(correlation_id: str) -> List[Dict]:
    ...
    import platformdirs
    app_data = platformdirs.user_data_dir("tim_mcp_server")
...
async def test_all_tools(server_script_path: str):
    ...
    server_params = StdioServerParameters(
        command="python",
        args=["-m", "tim_mcp_server.server.app", "--transport", "stdio"],
        env=env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            console.print("1. Run the Streamlit UI: [cyan]streamlit run tim_mcp_server/ui/app.py[/cyan]")
            ...
            db_path = Path.home() / ".local" / "share" / "tim_mcp_server" / "unified_logs.db"
...
@click.option(
    '--server-module',
    default="tim_mcp_server.server.app",
