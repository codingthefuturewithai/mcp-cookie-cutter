"""Configuration management page for Post-fix Test MCP Admin UI
...
try:
    from post_fix_test_mcp.ui.lib.components import (
        render_info_card,
        render_warning_banner,
        render_config_section
    )
    from post_fix_test_mcp.ui.lib.utils import (
        load_configuration,
...
def render_configuration_form(config):
    ...
    with st.form("config_form"):
        ...
        with col1:
            ...
            server_name = st.text_input("Server Name", value=server_config.get("name", "Post-fix Test MCP"))
            server_port = st.number_input("Server Port", 
                                        value=server_config.get("port", 3001),
                                        min_value=1, max_value=65535)
            # Get current log level from runtime config
            try:
                from post_fix_test_mcp.config import get_config
...
def handle_save_config(server_name, server_port, log_level, log_retention, current_config):
    ...
    new_config = {
        "server": {
            "name": server_name,
            "description": current_config.get("server", {}).get("description", "Post-fix Test MCP - MCP server with tools and integrations"),
            "port": server_port,
            "log_level": log_level,
...
def handle_export_config(config):
    ...
    st.download_button(
        ...
        data=config_json,
        file_name="post_fix_test_mcp_config.json",
...
def render_environment_variables():
    ...
    env_vars = {
        "POST_FIX_TEST_MCP_LOG_LEVEL": "INFO",
        "POST_FIX_TEST_MCP_PORT": "3001",
        "POST_FIX_TEST_MCP_CONFIG_PATH": system_info.get("config_path", "~/.config/post_fix_test_mcp"),
        "POST_FIX_TEST_MCP_LOG_PATH": system_info.get("log_path", "~/.local/share/post_fix_test_mcp"),
...
def main():
    ...
    # Page header
    st.title("⚙️ Post-fix Test MCP Configuration")
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=config_yaml,
            file_name="post_fix_test_mcp_config.yaml",
