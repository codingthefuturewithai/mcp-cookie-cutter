"""Post-fix Test MCP - MCP Server with Decorators
...
from post_fix_test_mcp.config import ServerConfig, get_config
from post_fix_test_mcp.logging_config import setup_logging, logger
from post_fix_test_mcp.log_system.correlation import (
    generate_correlation_id,
    set_initialization_correlation_id,
    clear_initialization_correlation_id
)
from post_fix_test_mcp.log_system.unified_logger import UnifiedLogger

from post_fix_test_mcp.tools.example_tools import example_tools, parallel_example_tools
...
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    from post_fix_test_mcp.log_system.destinations import DestinationConfig
    ...
    import logging
    unified_logger = logging.getLogger('post_fix_test_mcp')
    ...
    mcp_server = FastMCP(config.name or "Post-fix Test MCP")
...
def register_tools(mcp_server: FastMCP, config: ServerConfig) -> None:
    ...
    # Import decorators
    from post_fix_test_mcp.decorators.exception_handler import exception_handler
    from post_fix_test_mcp.decorators.tool_logger import tool_logger
    from post_fix_test_mcp.decorators.type_converter import type_converter
    from post_fix_test_mcp.decorators.parallelize import parallelize
...
def main(port: int, transport: str) -> int:
    """Run the Post-fix Test MCP server with specified transport."""
