...
{
  "post_fix_test_mcp": {
    "command": "/absolute/path/to/post_fix_test_mcp-server"
  }
}
```
...
1. Use `uvx post_fix_test_mcp-server` as the command
...
uv tool list | grep post_fix_test_mcp
# Or on macOS/Linux:
which post_fix_test_mcp-server
# Or on Windows:
where post_fix_test_mcp-server
```
...
{
  "post_fix_test_mcp": {
    "command": "/path/to/post_fix_test_mcp-server"
  }
}
```
...
```json
{
  "post_fix_test_mcp": {
    "command": "python",
    "args": ["-m", "post_fix_test_mcp.server.app"],
    "env": {
      "PYTHONPATH": "/path/to/post_fix_test_mcp"
    }
  }
}
```
...
# macOS/Linux:
which post_fix_test_mcp-server
# Example output: /Users/username/.local/bin/post_fix_test_mcp-server

# Windows:
where post_fix_test_mcp-server
# Example output: C:\Users\username\.local\bin\post_fix_test_mcp-server.exe
```
...
```bash
LOG_LEVEL=DEBUG uvx post_fix_test_mcp-server
```
