# Testing Guide for Post-fix Test MCP
...
uv run pytest tests/ --cov=post_fix_test_mcp --cov-report=term --cov-report=html
...
uv run pytest tests/ --cov=post_fix_test_mcp --cov-report=html
...
import pytest
from post_fix_test_mcp.tools.my_tools import my_new_tool
...
uv run pytest tests/ --cov=post_fix_test_mcp --cov-fail-under=40

# Generate coverage report for CI artifacts
uv run pytest tests/ --cov=post_fix_test_mcp --cov-report=xml
```
...
4. Check coverage hasn't decreased: `uv run pytest tests/ --cov=post_fix_test_mcp`
