...
def get_sqlite_logs(correlation_id: str) -> List[Dict]:
    ...
    import platformdirs
    app_data = platformdirs.user_data_dir("post_fix_test_mcp")
...
async def test_all_tools(server_script_path: str):
    ...
    server_params = StdioServerParameters(
        command="python",
        args=["-m", "post_fix_test_mcp.server.app", "--transport", "stdio"],
        env=env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            console.print("1. Run the Streamlit UI: [cyan]streamlit run post_fix_test_mcp/ui/app.py[/cyan]")
            ...
            db_path = Path.home() / ".local" / "share" / "post_fix_test_mcp" / "unified_logs.db"
...
@click.option(
    '--server-module',
    default="post_fix_test_mcp.server.app",
