# Post-fix Test MCP - Quick Setup Guide
...
uv tool install post_fix_test_mcp

# Find binary location
which post_fix_test_mcp-server  # macOS/Linux
where post_fix_test_mcp-server  # Windows
```
...
post_fix_test_mcp-server

# Start with SSE transport
post_fix_test_mcp-server --transport sse --port 3001

# Start with streamable HTTP transport
post_fix_test_mcp-server --transport streamable-http --port 3001

# Get help
post_fix_test_mcp-server --help
```

### Testing Commands
```bash
# Test client (stdio)
post_fix_test_mcp-client "Hello, World!"

# Run all tests
pytest

# Run with coverage
pytest --cov=post_fix_test_mcp
...
PYTHONPATH=. mcp dev post_fix_test_mcp/server/app.py

# Streamlit UI (monitoring interface)
streamlit run post_fix_test_mcp/ui/app.py

# Code formatting
black post_fix_test_mcp/
isort post_fix_test_mcp/
```
...
which post_fix_test_mcp-server
post_fix_test_mcp-server --help

# Check tools are registered
echo '{"method": "tools/list"}' | post_fix_test_mcp-server
```
...
# View logs
# macOS:
tail -f ~/Library/Logs/mcp-servers/post_fix_test_mcp.log
# Linux:
tail -f ~/.local/state/mcp-servers/logs/post_fix_test_mcp.log
# Windows PowerShell:
Get-Content -Wait "$env:LOCALAPPDATA\mcp-servers\logs\post_fix_test_mcp.log"
```

### Transport Debugging
```bash
# Test stdio (default)
echo '{"method": "ping"}' | post_fix_test_mcp-server
...
# Windows PowerShell:
Start-Job { post_fix_test_mcp-server --transport sse --port 3001 }
...
# Windows PowerShell:
Start-Job { post_fix_test_mcp-server --transport streamable-http --port 3001 }
...
- **MCP_SERVER_NAME**: Custom server name (default: "Post-fix Test MCP")
...
2. ✅ Test with `post_fix_test_mcp-client "test"`
...
This setup guide should get you productive with Post-fix Test MCP quickly!