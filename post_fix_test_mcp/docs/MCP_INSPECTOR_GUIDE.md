# MCP Inspector Testing Guide for Post-fix Test MCP
...
which mcp
# Should show: /path/to/post_fix_test_mcp/.venv/bin/mcp
...
1. Use the full path to your venv's mcp:
   ```bash
   .venv/bin/mcp dev post_fix_test_mcp/server/app.py
   ```
...
```bash
mcp dev post_fix_test_mcp/server/app.py
```
...
```bash
python -m post_fix_test_mcp.server.app --transport sse
```
...
```bash
python -m post_fix_test_mcp.server.app --transport streamable-http
```
...
**Text Logs**:
- macOS: `~/Library/Logs/mcp-servers/post_fix_test_mcp.log`
- Linux: `~/.local/state/mcp-servers/logs/post_fix_test_mcp.log`
- Windows: `%LOCALAPPDATA%\mcp-servers\logs\post_fix_test_mcp.log`

**SQLite Database** (tool execution history):
- macOS: `~/Library/Application Support/post_fix_test_mcp/tool_logs.db`
- Linux: `~/.local/share/post_fix_test_mcp/tool_logs.db`
- Windows: `%LOCALAPPDATA%\post_fix_test_mcp\tool_logs.db`
...
If you see `ModuleNotFoundError: No module named 'post_fix_test_mcp'`:

1. Ensure you're in the project root directory
2. Check virtual environment is activated: `which python`
3. Reinstall: `uv sync`
4. Use full mcp path: `.venv/bin/mcp dev post_fix_test_mcp/server/app.py`
...
1. Add your own tools in `post_fix_test_mcp/tools/`
