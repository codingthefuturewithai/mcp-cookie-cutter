...
3. **Verify installation:**
   ...
   post_fix_test_mcp-server --transport stdio
...
```
post_fix_test_mcp/
├── post_fix_test_mcp/           # Main package
│   ├── __init__.py
...
```python
from post_fix_test_mcp.log_system.destinations.factory import create_destination
...
2. **Register tool in server:**
   ```python
   # server/app.py
   from post_fix_test_mcp.tools.my_tool import my_tool
...
# tests/conftest.py
import pytest
from post_fix_test_mcp.config import ServerConfig
...
# Check formatting
black --check post_fix_test_mcp/
isort --check-only post_fix_test_mcp/

# Lint code
flake8 post_fix_test_mcp/
mypy post_fix_test_mcp/
```
...
# View logs
tail -f ~/.local/state/mcp-servers/logs/post_fix_test_mcp.log  # Linux
tail -f ~/Library/Logs/mcp-servers/post_fix_test_mcp.log       # macOS
...
- Use absolute imports: `from post_fix_test_mcp.tools import my_tool`
