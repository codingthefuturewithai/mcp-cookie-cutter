# Developer Guide - Post-fix Test MCP
...
```python
# post_fix_test_mcp/tools/my_tools.py
...
```python
from post_fix_test_mcp.tools.my_tools import my_tools, my_parallel_tools
```
...
```bash
# Start the inspector
uv run mcp dev post_fix_test_mcp/server/app.py
...
uv run pytest --cov=post_fix_test_mcp --cov-report=html
